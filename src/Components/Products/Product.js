...
import {
  deleteProductFromProductListAction,
  updateSpecificProductAction,
  updateUpdateCategoryStateAction,
  updateUpdatePlaceOrderStateAction,
...
const Product = () => {
  ...
  const deleteItemModal = () => {
    ...
    DELETE_PRODUCT_API(deleteObjInd)
      .then((response) => {
        dispatch(deleteProductFromProductListAction({ id: deleteObjInd }));
        customAlertModalFun(
          `Product ${deleteObjName} deleted successfully`,
          dispatch
        );
    ...
    <>
      ...
      <div>
        <Grid item xs={12}>
          ...
          >
            {(products.length > 0 ? products : productListViewState).map(
              (item, index) => (
                <Grid
                  key={item.id}
                  item
                ...
                >
                  <Card
                    sx={{ maxWidth: 345, width: 350 }}
                    onClick={(e) => placeOrderHandler(e, item)}
                    className="cursor-pointer"
                  >
                    <CardMedia sx={{ height: 245 }} image={item?.imageUrl} />
                    <CardContent
                      style={{
                        height: "90px",
                        maxHeight: "100px",
                    ...
                    <CardActions className="flex-row justify-content-between">
                      ...
                      {isUserAdmin && (
                        <div>
                          <CreateIcon
                            style={{ color: "#757575", marginRight: "16px" }}
                            className="cursor-pointer"
                            onClick={(e) =>
                              updateProductDetails(e, item, index)
                            }
                          />
                          <DeleteIcon
                            style={{ color: "#757575" }}
                            className="cursor-pointer"
                            onClick={(e) =>
                              deleteProductHanlder(e, item.id, item.name)
                            }
                          />
                        </div>
                      )}
      ...
      <div>
        <CustomModal
          ...
          confirmHandler={deleteItemModal}
